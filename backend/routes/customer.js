...
const PI_URL = 'http://172.20.10.2:4000';
...
router.post('/orders', async (req, res) => {
  try {
    ...
    fetch(`${PI_URL}/new-order`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ orderNumber: order.orderNumber, tableNumber: tableId }),
    }).catch(err => console.error('Pi buzzer not reachable:', err.message));
...
router.post('/sessions/:sessionId/pay', async (req, res) => {
  try {
    ...
    let discountAmount = 0, couponId = null, appliedCode = null;
    if (couponCode) {
...
// Helper — mark all pending orders served and clear from kitchen
const clearSessionOrders = async (sessionId, io) => {
  ...
  for (const order of pendingOrders) {
    order.status = 'served';
    await order.save();
    io.emit('kitchen:order_removed', { orderId: order._id });
  }
};
...
router.post('/sessions/:sessionId/upi-paid', async (req, res) => {
  try {
    ...
    await clearSessionOrders(session._id, req.io);
