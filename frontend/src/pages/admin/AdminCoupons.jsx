...
export default function AdminCoupons() {
  ...
  const confirmDelete = async () => {
    if (!del) return
    await api.delete(`/admin/coupons/${del._id}`)
    setDel(null)
    load()
  }
  ...
  return (
    <div className="p-7 space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="font-display text-3xl font-bold text-surface-900 italic">Coupons</h1>
              ...
              <tr key={c._id} className="hover:bg-surface-50 transition-colors">
                ...
                <td className="px-4 py-3">
                  <div className="flex items-center gap-1">
                    <button onClick={(e) => toggleActive(e, c)}>
                      {c.active
                    ...
                    <button onClick={(e) => handleDelete(e, c)}
      ...
      {del && (
        <Confirm
          ...
          title="Delete Coupon"
          message={`Permanently delete coupon "${del?.code}"? This cannot be undone.`}
          danger
        />
      )}
    </div>
