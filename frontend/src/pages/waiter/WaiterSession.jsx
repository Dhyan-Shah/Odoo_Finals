...
import { ArrowLeft, Clock, CheckCircle2, Receipt, PackageCheck } from 'lucide-react'
...
export default function WaiterSession() {
  ...
  const acceptOrder = async (orderId) => {
    await api.patch(`/waiter/orders/${orderId}/accept`)
    load()
  }
    ...
    <div className="p-4 max-w-2xl mx-auto space-y-4">
      ...
      <div className="flex items-center gap-3">
        ...
        <div className="text-right">
          ...
          {discountAmount > 0 && <p className="text-xs text-emerald-600">-{fmt(discountAmount)} discount</p>}
        </div>
      </div>
      ...
      {/* Ready orders — waiter must Accept before serving */}
      {readyOrders.length > 0 && (
        <div className="bg-emerald-50 border-2 border-emerald-300 rounded-2xl p-4">
          <p className="font-semibold text-emerald-800 text-sm mb-2">
            🍽️ {readyOrders.length} order{readyOrders.length > 1 ? 's' : ''} ready — pick up from kitchen
          </p>
          {readyOrders.map(order => (
            <div key={order._id} className="flex items-center justify-between py-1 gap-3">
              <span className="text-emerald-700 text-sm flex-1">
                {order.orderNumber} — {order.items.map(i => `${i.name}×${i.quantity}`).join(', ')}
              </span>
              <button onClick={() => acceptOrder(order._id)}
                className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-blue-500 text-white text-xs font-medium hover:bg-blue-600 transition-colors flex-shrink-0">
                <PackageCheck className="w-3.5 h-3.5" /> Accept Order
              </button>
            </div>
          ))}
        </div>
      ...
      {/* Accepted orders — waiter has picked up, now deliver and mark served */}
      {acceptedOrders.length > 0 && (
        <div className="bg-blue-50 border-2 border-blue-300 rounded-2xl p-4">
          <p className="font-semibold text-blue-800 text-sm mb-2">
            🚶 {acceptedOrders.length} order{acceptedOrders.length > 1 ? 's' : ''} picked up — deliver to table
          </p>
          {acceptedOrders.map(order => (
            <div key={order._id} className="flex items-center justify-between py-1 gap-3">
              ...
              <button onClick={() => markServed(order._id)}
                className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-emerald-500 text-white text-xs font-medium hover:bg-emerald-600 transition-colors flex-shrink-0">
      ...
      <div className="space-y-3">
        ...
        {orders.map(order => (
          <div key={order._id} className="card overflow-hidden">
            ...
            {/* Accept button in card */}
            {order.status === 'ready' && (
              <div className="px-4 pb-3">
                <button onClick={() => acceptOrder(order._id)}
                  className="w-full py-2 rounded-xl bg-blue-500 text-white text-sm font-medium hover:bg-blue-600 transition-colors flex items-center justify-center gap-2">
                  <PackageCheck className="w-4 h-4" /> Accept Order from Kitchen
                </button>
              </div>
            )}
            {/* Serve button in card */}
      ...
      {session.status === 'active' && orders.every(o => o.status === 'served') && orders.length > 0 && (
        <button onClick={() => setEndConfirm(true)}
          ...
          <Receipt className="w-4 h-4" /> End Session & Free Table
        </button>
      )}
